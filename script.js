...
document.addEventListener('DOMContentLoaded', () => {
    ...
    audio.addEventListener('timeupdate', () => {
        ...
        let batas1 = audio.duration / 3;
        let batas2 = (audio.duration / 3) * 2;
        
        if (audio.currentTime >= batas1 && audio.currentTime < batas2 && sequenceState === 0) {
            sequenceState = 1;
        ...
        else if (audio.currentTime >= batas2 && sequenceState === 1) {
            sequenceState = 2;
            if(temple1) temple1.emit('sembunyi'); 
            setTimeout(() => { 
                if(temple2) temple2.emit('muncul'); 
            }, 1000);
        }
    });
    ...
    function setDurasiPutaran() {
        ...
        // --- DI UBAH JADI 0 0 0 AGAR BERDIRI TEGAK ---
        ...
        if(temple2) temple2.setAttribute('animation', animasiBaru);
    }
    ...
    marker.addEventListener('markerFound', () => {
        ...
        if (isAudioFinished) {
            ...
            if(temple2) temple2.setAttribute('scale', '0 0 0');
    ...
    audio.onended = () => {
        ...
        if (markerVisible) {
            ...
            if(sequenceState === 2 && temple2) temple2.emit('sembunyi');
